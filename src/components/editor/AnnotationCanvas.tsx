...
/** Which crop handle (corner/edge/move) is under the point, or null. */
function cropHandleAt(point: Point, rect: CropRect, hit: number): string | null {
  const { x, y, width: w, height: h } = rect;
  const near = (px: number, py: number) => Math.abs(point.x - px) <= hit && Math.abs(point.y - py) <= hit;
  if (near(x, y)) return "nw";
  if (near(x + w, y)) return "ne";
  if (near(x, y + h)) return "sw";
  if (near(x + w, y + h)) return "se";
  const onV = (ex: number) => Math.abs(point.x - ex) <= hit && point.y > y + hit && point.y < y + h - hit;
  const onH = (ey: number) => Math.abs(point.y - ey) <= hit && point.x > x + hit && point.x < x + w - hit;
  if (onH(y)) return "n";
  if (onH(y + h)) return "s";
  if (onV(x)) return "w";
  if (onV(x + w)) return "e";
...
/** Apply a move/resize gesture to the crop rect, clamped to the canvas. */
function resizeCropRect(start: CropRect, handle: string, dx: number, dy: number, cw: number, ch: number): CropRect {
  if (handle === "move") {
    return {
      x: clampN(start.x + dx, 0, cw - start.width),
      y: clampN(start.y + dy, 0, ch - start.height),
      width: start.width,
      height: start.height,
    };
  }
  ...
  if (handle.includes("w")) left = clampN(start.x + dx, 0, right - CROP_MIN);
  if (handle.includes("e")) right = clampN(right + dx, left + CROP_MIN, cw);
  if (handle.includes("n")) top = clampN(start.y + dy, 0, bottom - CROP_MIN);
  if (handle.includes("s")) bottom = clampN(bottom + dy, top + CROP_MIN, ch);
  return { x: left, y: top, width: right - left, height: bottom - top };
}

function cropCursorFor(handle: string | null): string {
  switch (handle) {
    case "nw":
    case "se":
      return "nwse-resize";
...
export const AnnotationCanvas = memo(function AnnotationCanvas({
  ...
  // Crop tool state (canvas-pixel coordinates). Box persists after drawing so
  // it can be moved/resized via corner & edge handles before applying.
  const [cropRect, setCropRect] = useState<CropRect | null>(null);
  const cropActionRef = useRef<{ mode: "draw" | "move" | "resize"; handle: string; start: Point; startRect: CropRect } | null>(null);
  ...
  const createAnnotation = useCallback(
    (type: ToolType, start: Point, end: Point): Annotation | null => {
      if (!type || type === "select" || type === "crop") return null;

      const defaultColor = { hex: activeColor, opacity: 100 };
      const defaultBorder = { width: 5, color: { hex: activeColor, opacity: 100 } };
  ...
  const redraw = useCallback(() => {
    ...
    annotations.forEach((annotation) => {
      if (annotation.id === editingTextId) return;
    ...
    // Crop overlay: dim outside, thin outline, macOS-style corner brackets.
    if (selectedTool === "crop" && cropRect && cropRect.width > 1 && cropRect.height > 1) {
      const { x, y, width: w, height: h } = cropRect;
      ctx.save();

      // Dim the area being cropped away (kept area stays full brightness).
      ...
      // Thin border around the kept area.
      ctx.strokeStyle = "rgba(255, 255, 255, 0.9)";
      ctx.lineWidth = Math.max(1, canvas.width * 0.0012);
      ctx.strokeRect(x, y, w, h);

      // L-shaped corner brackets.
      const len = Math.max(14, Math.min(w, h) * 0.12);
      ctx.strokeStyle = "#ffffff";
      ctx.lineWidth = Math.max(3, canvas.width * 0.003);
      ctx.lineCap = "round";
      const bracket = (cx: number, cy: number, hx: number, hy: number) => {
        ctx.beginPath();
        ctx.moveTo(cx + hx * len, cy);
        ctx.lineTo(cx, cy);
        ctx.lineTo(cx, cy + hy * len);
        ctx.stroke();
      };
      bracket(x, y, 1, 1);
      bracket(x + w, y, -1, 1);
  ...
  useEffect(() => {
    if (selectedTool === "crop" && !cropRect && imageLoaded && imageRef.current) {
      const img = imageRef.current;
      setCropRect({ x: 0, y: 0, width: img.width, height: img.height });
    }
  }, [selectedTool, imageLoaded, cropRect]);

  // Crop confirm/cancel via keyboard. Capture phase so Escape doesn't
  // bubble to the editor's "close window" handler while cropping.
  useEffect(() => {
    if (selectedTool !== "crop") return;
    const onKey = (e: KeyboardEvent) => {
      if (e.target instanceof HTMLInputElement || e.target instanceof HTMLTextAreaElement) return;
      if (e.key === "Enter") {
        if (cropRect && cropRect.width > 1 && cropRect.height > 1 && onCrop) {
          e.preventDefault();
          e.stopPropagation();
          onCrop(cropRect);
          setCropRect(null);
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (selectedTool === "crop") {
      if (cropRect) {
        const canvas = canvasRef.current;
        const hit = canvas ? Math.max(24, Math.min(canvas.width, canvas.height) * 0.04) : 24;
        const handle = cropHandleAt(point, cropRect, hit);
        if (handle) {
      ...
      // Start a fresh box
      cropActionRef.current = { mode: "draw", handle: "", start: point, startRect: { x: point.x, y: point.y, width: 0, height: 0 } };
    ...
    if (selectedTool === "crop") {
      ...
      if (action) {
        ...
        if (action.mode === "draw") {
          ...
          setCropRect(resizeCropRect(action.startRect, action.handle, point.x - action.start.x, point.y - action.start.y, cw, ch));
        }
      } else if (canvas) {
        canvas.style.cursor = cropRect ? cropCursorFor(cropHandleAt(point, cropRect, Math.max(24, Math.min(canvas.width, canvas.height) * 0.04))) : "crosshair";
      }
      return;
    }
  ...
  }, [getCanvasCoordinates, resizingAnnotation, resizeHandle, resizeStartPoint, draggingAnnotation, dragOffset, annotations, isDrawing, startPoint, selectedTool, selectedAnnotation, applyResize, getHandleAtPoint, isPointInAnnotation, getCursorForHandle, onAnnotationUpdateTransient, onAnnotationUpdate, cropRect]);

  const handleMouseUp = () => {
    if (selectedTool === "crop") {
      const action = cropActionRef.current;
      cropActionRef.current = null;
      // Discard accidental tiny initial drags
      if (action?.mode === "draw") {
        setCropRect((r) => (r && r.width > 4 && r.height > 4 ? r : null));
      }
      return;
    }
  ...
  const handleDoubleClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const hit = [...annotations].reverse().find(
      (ann) => ann.type === "text" && isPointInAnnotation(point, ann)
    );
    if (hit) {
  ...
  const editingTextOverlay = (() => {
    const canvas = canvasRef.current;
    const container = containerRef.current;
    if (!editingText || editingText.type !== "text" || !canvas || !container) return null;
    const cRect = canvas.getBoundingClientRect();
    const conRect = container.getBoundingClientRect();
    const offsetX = cRect.left - conRect.left;
    const offsetY = cRect.top - conRect.top;
    const scale = cRect.width / canvas.width || 1;
    return {
      left: offsetX + editingText.x * scale,
      top: offsetY + editingText.y * scale,
      width: editingText.width * scale,
      height: editingText.height * scale,
      fontSize: editingText.fontSize * scale,
      fontFamily: editingText.fontFamily,
      color: editingText.fill.hex,
  ...
  const commitEditingText = (next: string) => {
    if (!editingText || editingText.type !== "text") return;
    onAnnotationUpdate({ ...editingText, text: next });
  };
  ...
  return (
    <div ref={containerRef} className="relative flex items-center justify-center w-full h-full min-w-0 min-h-0">
      ...
      {editingTextOverlay && (
        <textarea
          ref={editingTextRef}
          defaultValue={editingTextOverlay.value}
          onBlur={(e) => {
            commitEditingText(e.target.value);
          ...
          style={{
            ...
            height: editingTextOverlay.height,
            fontSize: editingTextOverlay.fontSize,
            fontFamily: editingTextOverlay.fontFamily,
