# SyncShot Hub + Mac Client (Phase 1) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** A self-hosted hub server (runs on the Mac mini) plus a Mac client agent (runs on the MacBook) that sync screenshots between Macs through the hub — independent of iCloud — with the existing SyncShot Tauri app surfacing + clipboard-copying received shots via its folder poll.

**Architecture:** Hub = FastAPI + SQLite catalog + content-addressed blob store + WebSocket fan-out. Mac client = folder watcher (upload) + WebSocket receiver (writes received shots into the configured SyncShot folder). Dedup by SHA-256. Phase 1 deliberately excludes FCM/Android (Phase 2) so there is **no Firebase dependency** and everything is testable on `localhost`.
...
## File structure (new repo: `/Users/aakashnarukula/Developer/syncshot-hub`)
...
syncshot-hub/
  ...
  src/ssx_hub/
    __init__.py
    config.py                   # HubConfig: paths, port, configured SyncShot folder
...
**Files:**
- Create: `/Users/aakashnarukula/Developer/syncshot-hub/pyproject.toml`
- Create: `src/ssx_hub/__init__.py`, `src/ssx_client/__init__.py`, `tests/conftest.py`

- [ ] **Step 1: Create repo + venv + deps**

```bash
mkdir -p /Users/aakashnarukula/Developer/syncshot-hub/{src/ssx_hub,src/ssx_client,tests,deploy}
cd /Users/aakashnarukula/Developer/syncshot-hub
git init -q
python3 -m venv .venv
...
def workdir(tmp_path):
    folder = tmp_path / "SyncShot"; folder.mkdir()
...
@dataclass
class HubConfig:
    folder: pathlib.Path     # the user-visible SyncShot folder (configurable, may move)
    ...
    def from_env(cls, base: "pathlib.Path | None" = None) -> "HubConfig":
        base = pathlib.Path(base or (pathlib.Path.home() / "Library/Application Support/SyncShotHub"))
        base.mkdir(parents=True, exist_ok=True)
        folder = pathlib.Path(os.environ.get("SSX_FOLDER", pathlib.Path.home() / "Desktop/SyncShot"))
...
def create_app(cfg: HubConfig) -> FastAPI:
    app = FastAPI(title="SyncShot Hub")
    app.state.cfg = cfg
...
ROOT="/Users/aakashnarukula/Developer/syncshot-hub"
export SSX_FOLDER="${SSX_FOLDER:-$HOME/Desktop/SyncShot}"
...
DEFAULT_FOLDER = str(pathlib.Path.home() / "Desktop/SyncShot")
...
<plist version="1.0"><dict>
  ...
  <array><string>/Users/aakashnarukula/Developer/syncshot-hub/deploy/run-hub.sh</string></array>
...
2. cloudflared tunnel create syncshot         # note the tunnel UUID + creds json
3. cloudflared tunnel route dns syncshot syncshot.gyftalala.com
4. Create ~/.cloudflared/config.yml:
     tunnel: <UUID>
     credentials-file: /Users/<you>/.cloudflared/<UUID>.json
     ingress:
       - hostname: syncshot.gyftalala.com
...
7. Verify:  curl https://syncshot.gyftalala.com/api/health   →  {"ok": true}
```
...
- **Phase 2:** `syncshot-android` (Kotlin/Compose, FCM, observer, WorkManager, gallery, notify-to-copy) — separate plan + needs Firebase project.
