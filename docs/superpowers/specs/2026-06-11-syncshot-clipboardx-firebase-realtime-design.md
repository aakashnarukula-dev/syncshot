# SyncShot v2 + ClipboardX — Firebase Realtime Sync (Design Spec)
...
1. **SyncShot** — capture on any device → appears on all paired devices in <1s. Permanent cloud history.
2. **ClipboardX** — every copied text is captured, stored, and synced realtime; re-copy any past entry. Two-section nav (SyncShot / ClipboardX) on both apps.
...
- Android app: `com.app.syncshot` (uses `android/app/google-services.json`, gitignored).
...
- **Mac:** on full → auto-save to SyncShot folder + copy image to clipboard (existing Rust path).
...
- **UI:** left-column nav, two sections **SyncShot** (existing grid) / **ClipboardX** (`src/components/ClipboardX/` list). Pairing view (show QR + code / enter code). Follow `AGENTS.md` UI rules (Tailwind defaults, Radix, no gratuitous motion, `h-dvh`).
...
- **Bottom nav:** SyncShot | ClipboardX. Keep `applicationId = com.app.syncshot`.
...
- Ensure `android/app/google-services.json` matches `com.app.syncshot` (re-download if needed).
