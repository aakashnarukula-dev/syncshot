# SyncShot Multi-Device Sync — Design Spec
...
Desktop syncing the `~/Desktop/SyncShot` folder) with an explicit, account-based,
...
```
                         ...
                         │  cloudflared (LaunchAgent)  syncshot.gyftalala.com → 127.0.0.1:8787     │
                         ...
                         │    • watches local SyncShot folder (ingest mini captures)                 │
         ...
         │   • watches local SyncShot folder      │      │   • FG service + MediaStore observer → upload │
...
- **Storage**
  ...
  - Display folder mirror: also writes a copy into the configured `SyncShot` folder so
...
- Watches the configured `SyncShot` folder → hashes new shots → `POST /api/upload`.
...
1. **Cloudflare named tunnel** for `syncshot.gyftalala.com` → `mini:8787`:
   `cloudflared tunnel login` (browser), `cloudflared tunnel create syncshot`,
...
- `syncshot` (existing Tauri app) — **minimal/no change** in Phase 1–2 (clipboard + column
  already work via the folder poll). Hub UI for pairing may be added later (Phase 3).
- **new**: `syncshot-hub` — hub server + Mac client agent (Python).
- **new**: `syncshot-android` — Kotlin/Compose app.
